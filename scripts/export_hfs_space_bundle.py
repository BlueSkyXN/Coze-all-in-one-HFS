...
PROFILE_PATH = "hfs-dev.toml"
RESERVED_PATHS = GENERATED_PATHS | {PROFILE_PATH}
RUNTIME_SOURCE_RE = re.compile(r"(?m)^ARG COZE_SOURCE_COMMIT=([0-9a-f]{40})$")
...
def load_config() -> dict[str, Any]:
    ...
    profile_manifests: set[str] = set()
    for name, profile in profiles.items():
        if not isinstance(name, str) or not isinstance(profile, dict):
            raise BundleError("profiles contain an invalid entry")
        manifest = _safe_relative(str(profile.get("manifest", "")), label="profile manifest")
        if manifest in profile_manifests:
            raise BundleError("profile manifests must be unique")
        profile_manifests.add(manifest)
    ...
    for source, destination in mapping.items():
        ...
        if source in sources or destination in destinations or destination in RESERVED_PATHS:
            raise BundleError("source_to_bundle contains a duplicate or reserved path")
        sources.add(source)
        destinations.add(destination)
    if "Dockerfile" not in destinations:
        raise BundleError("source_to_bundle must provide the bundle Dockerfile")
    return config
...
def expected_paths(config: dict[str, Any]) -> set[str]:
    return set(config["source_to_bundle"].values()) | RESERVED_PATHS
...
def runtime_source_commit(dockerfile: str) -> str:
    match = RUNTIME_SOURCE_RE.search(dockerfile)
    if match is None:
        raise BundleError("Dockerfile must declare one immutable COZE_SOURCE_COMMIT")
...
def export_bundle(source_commit: str, profile_name: str, output: Path) -> None:
    ...
    manifest_payload = blob(source_commit, selected_manifest)
    manifest_mode = tree_mode(source_commit, selected_manifest)
    validate_manifest(manifest_payload, profile["space"])
    write_file(output / PROFILE_PATH, manifest_payload, manifest_mode)
    source_entries.append(source_entry(selected_manifest, PROFILE_PATH, manifest_payload, manifest_mode))

    runtime_commit = runtime_source_commit((output / "Dockerfile").read_text(encoding="utf-8"))
...
def validate_dockerfile_sources(dockerfile: str, files: set[str]) -> None:
    ...
    for number, raw_line in enumerate(dockerfile.splitlines(), start=1):
        ...
        pending = ""
    if pending:
        raise BundleError(f"Dockerfile line {start} has an incomplete continuation")
...
def verify_bundle(bundle: Path, profile_name: str) -> None:
    ...
    validate_manifest((bundle / PROFILE_PATH).read_bytes(), profile["space"])
    try:
    ...
    if (
        ...
        or evidence.get("lane") != "artifact"
        or evidence.get("version_source") != "commit"
    ...
    if (
        evidence.get("runtime_source_repository") != RUNTIME_SOURCE_REPOSITORY
